            ...
            */
            ...
            while (true)
            {
                ...
                if (match.Success && match.Index == m_Offset)
                {
                    ...
                    try
                    {
                        if (String2.Equals(match.Groups["encoding"].Value, "Q", StringComparison2.InvariantCultureIgnoreCase))
                        {
                            retVal.Append(MIME_Utils.QDecode(Encoding.GetEncoding(match.Groups["charset"].Value), match.Groups["value"].Value));
                        }
                        else if (String2.Equals(match.Groups["encoding"].Value, "B", StringComparison2.InvariantCultureIgnoreCase))
                        {
                            retVal.Append(Encoding.GetEncoding(match.Groups["charset"].Value).GetString(Net_Utils.FromBase64(Encoding2.Default.GetBytes(match.Groups["value"].Value))));
                        }

    ...
    public class Dns_Client : IDisposable
    {
                ...
                if (m_HostNameOrIP.IndexOf(".") == -1)
                {
                    try
                    {
                        ...
                        AsyncCallback callback = delegate (IAsyncResult ar)
                        {
                            try
                            {
                                foreach (IPAddress ip in System.Net.Dns2.EndGetHostAddresses(ar))
                                {
                                    if (ip.AddressFamily == AddressFamily.InterNetwork)
                                    {
                        ...
                        System.Net.Dns2.BeginGetHostAddresses(m_HostNameOrIP, callback, null);
                    }
                    catch (Exception x)
                    {
                        m_pException = x;
        ...
        public class GetEmailHostsAsyncOP : IDisposable, IAsyncOP
        {
            ...
            private IPAddress[] Get_A_or_AAAA_FromResponse(string name, DnsServerResponse response)
            {
                ...
                {
                    if (String2.Equals(name, rr.Name, StringComparison2.InvariantCultureIgnoreCase))
                    {
                        if (rr is DNS_rr_A)
                        {
        ...
        internal static string ReadCharacterString(byte[] data, ref int offset)
        {
            ...
            */
            ...
            string retVal = Encoding2.Default.GetString(data, offset, dataLength);
            offset += dataLength;
        ...
        public static IPAddress[] Resolve(string host)
        {
            ...
            if (host.IndexOf(".") == -1)
            {
                return System.Net.Dns2.GetHostEntry(host).AddressList;
            }
            else
            {

...
namespace LumiSoft.Net.TCP
{
    ...
    public class TCP_ServerSession : TCP_Session
    {
        ...
        private void LogAddText(string text)
        {
            ...
            try
            {
                object logger = System.Reflection.IntrospectionExtensions.GetTypeInfo(this.Server.GetType())
                    .GetProperty("Logger")
                    .GetValue(this.Server, null);
        ...
        protected virtual void OnError(Exception x)
        {
            if (this.Error != null)
            {
                this.Error(this, new Error_EventArgs(x, new System.Diagnostics.StackTrace(x, false)));
            }
        }

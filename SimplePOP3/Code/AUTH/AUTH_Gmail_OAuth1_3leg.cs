            ...
            using(WebResponse response = request.GetResponse()){
                using(StreamReader reader = new StreamReader(response.GetResponseStream())){
                    foreach(string parameter in HttpUtility.UrlDecode(reader.ReadToEnd()).Split('&')){
                        ...
                        if(string.Equals(name_value[0],"oauth_token",StringComparison2.InvariantCultureIgnoreCase)){
                            m_RequestToken = name_value[1];
                        }
                        else if(string.Equals(name_value[0],"oauth_token_secret",StringComparison2.InvariantCultureIgnoreCase)){
            ...
            using(WebResponse response = request.GetResponse()){
                using(StreamReader reader = new StreamReader(response.GetResponseStream())){
                    foreach(string parameter in HttpUtility.UrlDecode(reader.ReadToEnd()).Split('&')){
                        ...
                        // string.Equals(name_value[0],"email", StringComparison2.InvariantCultureIgnoreCase)
                        if (StringComparison2.Equals(name_value[0], "email"))
                        {

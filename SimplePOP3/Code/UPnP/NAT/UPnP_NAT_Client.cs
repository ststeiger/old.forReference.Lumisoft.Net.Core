...
namespace LumiSoft.Net.UPnP.NAT
{
    ...
    public class UPnP_NAT_Client
    {
            ...
            try
            {
                ...
                if (devices.Length > 0)
                {
                    ...
                    while (queue.Count > 0)
                    {
                        ...
                        if (String2.Equals("urn:schemas-upnp-org:service:WANPPPConnection:1", currentNode.InnerText, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            foreach (XmlNode node in currentNode.ParentNode.ChildNodes)
                            {
                                if (String2.Equals("controlURL", node.Name, StringComparison2.InvariantCultureIgnoreCase))
                                {
                        ...
                        else if (String2.Equals("urn:schemas-upnp-org:service:WANIPConnection:1", currentNode.InnerText, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            foreach (XmlNode node in currentNode.ParentNode.ChildNodes)
                            {
                                if (String2.Equals("controlURL", node.Name, StringComparison2.InvariantCultureIgnoreCase))
            ...
            */
            ...
            for (int i = 0; i < 100; i++)
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        if (String2.Equals("NewRemoteHost", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            remoteHost = reader.ReadString();
                        }
                        else if (String2.Equals("NewExternalPort", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            externalPort = reader.ReadString();
                        }
                        else if (String2.Equals("NewProtocol", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            protocol = reader.ReadString();
                        }
                        else if (String2.Equals("NewInternalPort", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            internalPort = Convert.ToInt32(reader.ReadString());
                        }
                        else if (String2.Equals("NewInternalClient", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            internalHost = reader.ReadString();
                        }
                        else if (String2.Equals("NewEnabled", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            enabled = Convert.ToBoolean(Convert.ToInt32(reader.ReadString()));
                        }
                        else if (String2.Equals("NewPortMappingDescription", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
                            description = reader.ReadString();
                        }
                        else if (String2.Equals("NewLeaseDuration", reader.Name, StringComparison2.InvariantCultureIgnoreCase))
                        {
        ...
        {
            throw new NotImplementedException("UPnP_NAT_Client.cs => SendCommand");

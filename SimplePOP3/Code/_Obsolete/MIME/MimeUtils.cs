    ...
    public class MimeUtils
    {
        ...
        public static string ParseHeaders(Stream entryStrm)
        {
			...
			*/
            ...
            return System.Text.Encoding2.Default.GetString(msHeaders.ToArray());
        }
        ...
        public static string ParseHeaderField(string fieldName, string headers)
        {
			...
			*/
            ...
            using (TextReader r = new StreamReader(new MemoryStream(System.Text.Encoding2.Default.GetBytes(headers))))
            {
                ...
                if (r.StartsWith("=?") && r.SourceString.IndexOf("?=") > -1)
                {
                    ...
                    try
                    {
                        ...
                        if (r.StartsWith("="))
                        {
                            ...
                            {
                                decodedWord = c.GetString(Core.Base64Decode(Encoding2.Default.GetBytes(encodedText)));
                            }
                        }
                    }
                    catch
